...
/**
 * Gemini 入力用に画像を 2000px 以下に縮小して JPEG 化。
 * 既に小さい画像はそのまま返す。
 *
 * 1600px だと「荻/外/佐」「寛/宣」のような似形漢字の判別余地が足りず
 * 同じカードを複数回スキャンすると毎回違う字を返すケースが頻発した。
 * 2000px にして点・払い・冠の細部をモデルに見せる。コスト増は数銭/枚程度。
 */
...
【最重要原則 ─ 厳守してください】
1. 推測や補完を絶対にしないでください。書かれていない情報を勝手に作らないでください。
2. 自信がない文字は空文字 "" にし、その項目の confidence を 0.3 以下にしてください。
3. 自信がない真偽値は必ず false にしてください。「そうかもしれない」では true にしないでください。
4. 同じ画像に対する応答が毎回同じになるよう、見えるものだけを忠実に読み取ってください。

【漢字の判別 ─ 形が似た字に特に注意】
- 「荻」「萩」「萩」「外」「佐」は別の字です。草冠の有無、点の数、人偏の有無を一画ずつ確認してください。
- 「寛」「宣」「冨」「富」も別字です。冠の中の形を慎重に。
- 市区町村名で似た名前があります（例:「志木市」と「さいたま市」、「府中市」が東京都/広島県、など）。
  推測せず、そこに書かれている字を一画ずつ読み取り、判読困難なら空文字にしてください。
- 一画でも不明瞭なら confidence を下げてください。
...
【チェックボックスの判別ルール ─ 厳密に】
- 空欄 (□、つまり中に何もない四角) は必ず false。
- ■、✓、●、塗りつぶし、明確な太い線が四角の "中に" 描かれている場合のみ true。
- 項目名のラベル「供花」「供物」「弔電」が印刷されているだけでは true にしないでください。
  チェックボックス自体に物理的に印がある時のみ true です。
- 薄い線、にじみ、紙のしわ、印字のかすれ、判断に迷う場合は必ず false にしてください。
- 用紙のレイアウト上ボックスがあるからといって埋めようとしないでください。空のままが正解です。
